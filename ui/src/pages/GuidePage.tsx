        ...
        <section className="mb-14">
          ...
          <div className="rounded-2xl border p-6" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--surface-2)' }}>
            ...
            <p className="mb-3 text-sm" style={{ color: 'var(--text-dim)' }}>
              Run this once on your machine to bootstrap necessary underlying tools. It walks you through three interactive steps:
            </p>
            
            <div className="mb-4 pl-4 border-l-2 border-white/10">
              <p className="mb-2 text-sm text-white">Step 1 — Methodology</p>
              <ul className="mb-4 list-disc pl-5 text-sm" style={{ color: 'var(--text-dim)' }}>
                <li><strong>Spec Driven Development (SDD):</strong> Architect-first workflow using Spec Kit</li>
                <li><strong>Agent Driven Development (ADD):</strong> Agent-first workflow using Agent Forge</li>
              </ul>
              
              <p className="mb-2 text-sm text-white">Step 2 — Execution Environment</p>
              <ul className="mb-4 list-disc pl-5 text-sm" style={{ color: 'var(--text-dim)' }}>
                <li><strong>CLI Toolchains:</strong> Global Node/Python packages (specify-cli, agent-forge-copilot)</li>
                <li><strong>IDE Native:</strong> Zero global dependencies (directly scaffolds rules into .github/.cursor)</li>
              </ul>

              <p className="mb-2 text-sm text-white">Step 3 — Headroom-AI</p>
              <p className="mb-2 text-sm" style={{ color: 'var(--text-dim)' }}>
                Optionally enable transparent prompt compression to reduce context window token usage by 60-95%.
              </p>
            </div>

            <CodeSnippet command="origin setup" className="mb-4" />
            
            <p className="mb-2 text-sm font-semibold" style={{ color: 'var(--text)' }}>Non-interactive flags (Automation)</p>
            <CodeSnippet command="origin setup --spec --copilot --headroom" className="mb-8" />

            <h3 className="mb-3 text-lg font-semibold" style={{ color: 'var(--text)' }}>Project Initialization</h3>
            <p className="mb-3 text-sm" style={{ color: 'var(--text-dim)' }}>
              Run this inside any project directory to initialize the AI workspace based on your setup preferences. If you chose "CLI Toolchains", it orchestrates <code>forge init</code> and <code>specify init</code>. If you chose "IDE Native", it bypasses CLI tooling and scaffolds local agents natively.
            </p>
            <CodeSnippet command="origin init" className="mb-4" />
            
            <p className="mb-2 text-sm font-semibold" style={{ color: 'var(--text)' }}>Advanced Flags</p>
            <div className="flex flex-col gap-4 mb-4">
              <div>
                <p className="text-sm mb-1" style={{ color: 'var(--text-dim)' }}>Force IDE Native initialization for a specific editor:</p>
                <CodeSnippet command="origin init --ide [copilot|vscode|claude|cursor]" className="!mb-0" />
              </div>
              <div>
                <p className="text-sm mb-1" style={{ color: 'var(--text-dim)' }}>Apply integration overrides (e.g., sync workflows to Jira issues):</p>
                <CodeSnippet command="origin init --extension jira" className="!mb-0" />
              </div>
            </div>
          </div>
        </section>
